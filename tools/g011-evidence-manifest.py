...
def tracked_files(root: Path, relative_dir: Path) -> frozenset[str]:
    tracked = {
        line.strip()
        for line in run_git(root, "ls-files", "--", relative_dir.as_posix()).splitlines()
        if line.strip()
    }
...
def build_manifest(root: Path, mode: str = "runtime") -> dict[str, object]:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--mode", choices=("baseline", "runtime"), default="runtime")
