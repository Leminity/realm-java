...
OFFICIAL_INVALID_THREAD_CATEGORY="$WORK/official-invalid-thread-category.txt"
...
"$ADB" -s "$SERIAL" exec-out run-as "$OFFICIAL_PACKAGE" cat files/ac08/invalid-thread-category.txt > "$OFFICIAL_INVALID_THREAD_CATEGORY"
SCHEMA_HASH="$(sha256sum "$OFFICIAL_SCHEMA" | awk '{print $1}')"
CATEGORY="$(tr -d '\r\n' < "$OFFICIAL_CATEGORY")"
INVALID_THREAD_CATEGORY="$(tr -d '\r\n' < "$OFFICIAL_INVALID_THREAD_CATEGORY")"
[[ "$CATEGORY" == 'io.realm.exceptions.RealmMigrationNeededException' ]] || fail "unexpected official no-migration category: $CATEGORY"
[[ "$INVALID_THREAD_CATEGORY" == 'java.lang.IllegalStateException' ]] || fail "unexpected official invalid-thread category: $INVALID_THREAD_CATEGORY"
printf 'schema_sha256=%s\nofficial_no_migration_category=%s\nofficial_invalid_thread_category=%s\n' \
  "$SCHEMA_HASH" "$CATEGORY" "$INVALID_THREAD_CATEGORY" | tee "$EVIDENCE/export-official-schema.log"
...
# Phase A/B demonstrates process death and a distinct PID before the fork instrumentation starts.
run phase-a-start "$ADB" -s "$SERIAL" shell am start -W -n "$FORK_PACKAGE/.ProbeActivity"
PID_A="$("$ADB" -s "$SERIAL" shell pidof "$FORK_PACKAGE" | tr -d '\r')"
[[ -n "$PID_A" ]] || fail 'phase A did not produce a fork process PID'
...
run phase-b-start "$ADB" -s "$SERIAL" shell am start -W -n "$FORK_PACKAGE/.ProbeActivity"
PID_B="$("$ADB" -s "$SERIAL" shell pidof "$FORK_PACKAGE" | tr -d '\r')"
[[ -n "$PID_B" && "$PID_B" != "$PID_A" ]] || fail "phase B PID must differ from phase A (A=$PID_A B=$PID_B)"
...
run fork-runtime "$ADB" -s "$SERIAL" shell am instrument -w -r \
  ...
  -e official_invalid_thread_category "$INVALID_THREAD_CATEGORY" \
