...
- Release-candidate tag input: `v10.19.0-agp9.3`.
- Immutable predecessor: tag `v10.19.0-agp9.2`.
...
- Version: `10.19.0-agp9.3` (`version.txt`).
...
Realm reflects over classes annotated with `@RealmModule`. Before `10.19.0-agp9.2`, the AAR
consumer metadata kept those classes but did not retain their constructors, so R8 could remove
the constructor needed by Realm at runtime. `10.19.0-agp9.2` introduced the exact
`@RealmModule` constructor keep rule in the Realm AAR, and `10.19.0-agp9.3` carries it forward.
After upgrading to `.3`, consumers should remove any equivalent application-local workaround; a
broad package keep is neither needed nor part of this release.
...
The `.3` tag may be created only after its exact source commit has passed the release gates and
been sealed as the tag input. Once created, it is immutable. The existing
`v10.19.0-agp9.2` tag remains immutable, and the published `10.19.0-agp9.1` artifacts must not
